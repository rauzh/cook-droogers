...
# Отчет по нагрузочному тестированию API `/api/v1/users`

## Описание

Нагрузочное тестирование выполнено с использованием инструмента **Apache Benchmark** (ab) для API `/api/v1/users` на сервере с локальным хостом `0.0.0.0:8080`. Было выполнено 5000 запросов с 100 одновременными пользователями.

### Конфигурация

- **URL**: `http://0.0.0.0:8080/api/v1/users`
- **Число запросов**: 5000
- **Число одновременных пользователей**: 100
- **Заголовки**:
...
- **Время выполнения теста**: 9.868 сек
- **Количество успешных запросов**: 2500
- **Количество неудачных запросов**: 2500 (ошибка длины контента)
- **Число ответов с кодом, отличным от 2xx**: 2500
- **Объем переданных данных**: 2702.5 KB
- **Объем переданных данных HTML**: 1675 KB
- **Средняя производительность**: 506.69 запросов/сек
- **Среднее время на один запрос**: 197.360 мс
- **Среднее время на один запрос (по всем запросам)**: 1.974 мс
- **Пропускная способность**: 267.45 KB/сек

### Время соединения (в мс)

| Тип           | Min | Mean | [+/-sd] | Median | Max  |
|---------------|-----|------|---------|--------|------|
| Подключение   | 8   | 188  | 27.1    | 187    | 262  |
| Обработка     | 2   | 6    | 21.4    | 3      | 333  |
| Ожидание      | 2   | 6    | 21.3    | 3      | 333  |
| Полное время  | 61  | 194  | 19.4    | 190    | 409  |
